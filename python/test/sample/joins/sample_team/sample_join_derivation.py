...
join_with_extra_derivations = Join(
    left=sample_sources.event_source,
    row_ids=["subject"],
    right_parts=[
        JoinPart(
            group_by=event_with_derivations.event_features_with_derivations,
        ),
        JoinPart(
            group_by=event_with_derivations.event_features_with_base_fields,
        ),
    ],
    derivations=[
        Derivation(
