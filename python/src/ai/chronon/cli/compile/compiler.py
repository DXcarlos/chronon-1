    ...
    def _table_uses(self, obj: Any) -> List[Tuple[str, bool, str]]:
        ...
        def add_source(source, field_prefix: str):
            ...
            if source.events:
                ...
                uses.append((source.events.table, query is not None and query.partitionInterval is not None, field_prefix))
        ...
        if isinstance(obj, (GroupBy, ModelTransforms)):
            for source in obj.sources or []:
                add_source(source, "source table")
        elif isinstance(obj, Join):
            add_source(obj.left, "left source table")
            for bootstrap in obj.bootstrapParts or []:
                query = bootstrap.query
                uses.append((bootstrap.table, query is not None and query.partitionInterval is not None, "bootstrap table"))
        elif isinstance(obj, StagingQuery):
            for dep in obj.tableDependencies or []:
                table_info = dep.tableInfo
                if table_info is not None:
                    uses.append((table_info.table, table_info.partitionInterval is not None, "table dependency"))
