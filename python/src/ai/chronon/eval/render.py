...
def render_join_result(result) -> str:
    ...
    try:
        ...
        if join_parts and len(join_parts) > 0:
    ...
    # Output schemas (only show if we have schemas to display)
    schema_lines = []
    ...
    try:
        ...
        if left_schema:
            ...
            schema_lines.append("  🔵 Left Schema:")
            schema_lines.append(render_schema_dict(left_schema_dict, indent=2))
            schema_lines.append("")
    except Exception as e:
        schema_lines.append(f"  ❌ Error reading left schema: {str(e)}")
        schema_lines.append("")
    ...
    try:
        ...
        if right_schema:
            ...
            schema_lines.append("  🟢 Right Schema:")
            schema_lines.append(render_schema_dict(right_schema_dict, indent=2))
            schema_lines.append("")
    except Exception as e:
        schema_lines.append(f"  ❌ Error reading right schema: {str(e)}")
        schema_lines.append("")
    ...
    try:
        ...
        if post_deriv_schema:
            ...
            schema_lines.append("  🔄 Post-Derivations Schema:")
            schema_lines.append(render_schema_dict(post_deriv_schema_dict, indent=2))
            schema_lines.append("")
    except Exception:
    ...
    try:
        ...
        if external_schema:
            ...
            schema_lines.append("  🔗 External Parts Schema:")
            schema_lines.append(render_schema_dict(external_schema_dict, indent=2))
            schema_lines.append("")
    except Exception:
        pass  # External schema is optional
    
    # Only add the Output Schema header if we have actual schema content
    if schema_lines:
        lines.append("📊 Output Schema:")
