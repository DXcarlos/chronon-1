...
class DateMacroSpec extends AnyFlatSpec with Matchers {
  ...
  // sub-daily specs: offset means N partition steps; labels render in the spec's own format
  it should "render sub-daily labels and step-denominated offsets" in {
    ...
    // offset=-2 steps back two 3h partitions, wrapping through the 01:00 anchor
    result shouldBe
