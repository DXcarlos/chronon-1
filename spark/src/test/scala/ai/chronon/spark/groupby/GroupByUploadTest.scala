...
class GroupByUploadTest extends SparkTestBase with Matchers {
  ...
  def runAndValidateActualTemporalBatchData(sparkSession: SparkSession, tableUtils: TableUtils, eventsTable: String): Array[Array[Any]] = {
    // Setup data
    createDatabase(namespace)
    tableUtils.sql(s"USE $namespace")
    ...
    tailHops(2) shouldBe empty
    tailHops
  }

  it should "produce valid batch data for temporal events case with spark.chronon.group_by.upload.combine disabled" in {
    // Create new spark session with addition spark conf set
    val mergedConfig = DefaultSparkConfs ++ Map(
            "spark.chronon.group_by.upload.combine" -> "false",
    )
    val updatedSparkSession =
      SparkSessionBuilder.build(
        getClass.getSimpleName,
        local = true,
        additionalConfig = Option(mergedConfig))

    val tableUtils = TableUtils(updatedSparkSession)

    val eventsTable = "my_events_check_temporal_combine_disabled"
    runAndValidateActualTemporalBatchData(sparkSession = updatedSparkSession, tableUtils = tableUtils, eventsTable = eventsTable)
  }
