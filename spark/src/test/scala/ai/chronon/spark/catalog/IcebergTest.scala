...
class IcebergTest extends SparkTestBase with Matchers {
  ...
  it should "report the sub-daily tail interval once each chunk lands after its interval closes" in {
    val tableName = "default.iceberg_chunked_write_test"
    spark.sql(s"DROP TABLE IF EXISTS $tableName")
    ...
    // two 3h chunks landing as separate append snapshots, both long after their intervals
    // closed — the shape of a chunked batch writer catching up
    spark.sql(s"""
      INSERT INTO $tableName VALUES
      (1, TIMESTAMP '2024-04-02 00:30:00'),
      (2, TIMESTAMP '2024-04-02 02:45:00')
    """)
    spark.sql(s"""
      INSERT INTO $tableName VALUES
      (3, TIMESTAMP '2024-04-02 03:15:00'),
      (4, TIMESTAMP '2024-04-02 05:59:00')
    """)

    Iceberg.lastAvailablePartition(tableName, "created_at", threeHourSpec) shouldBe Some("2024-04-02-03-00")
    Iceberg.virtualPartitions(tableName, "created_at", threeHourSpec) shouldBe
      List("2024-04-02-00-00", "2024-04-02-03-00")
  }

  it should "keep interval-end readiness while the Iceberg tail interval is still open" in {
    val tableName = "default.iceberg_open_interval_test"
    spark.sql(s"DROP TABLE IF EXISTS $tableName")
    ...
    // data in the next interval: it cannot close during the test, so the snapshot always
    ...
    spark.sql(s"INSERT INTO $tableName VALUES (1, TIMESTAMP '$openTs')")

    val dataBearing = threeHourSpec.at(openIntervalMillis)
    Iceberg.lastAvailablePartition(tableName, "created_at", threeHourSpec) shouldBe
      Some(threeHourSpec.before(dataBearing))
  }

  it should "keep interval-end readiness on Iceberg v1 tables where files cannot be attributed to commits" in {
    val tableName = "default.iceberg_v1_readiness_test"
    spark.sql(s"DROP TABLE IF EXISTS $tableName")

    val threeHourSpec = PartitionSpec("ds", "yyyy-MM-dd-HH-mm", 3 * 60 * 60 * 1000)

    // v1 tables have no usable sequence numbers, so a chunk-shaped write cannot be proven
    // complete and readiness stays one interval behind
    spark.sql(s"""
      CREATE TABLE $tableName (
        id INT,
        created_at TIMESTAMP
    ...
    spark.sql(s"""
      INSERT INTO $tableName VALUES
      (1, TIMESTAMP '2024-04-02 09:17:00'),
      (2, TIMESTAMP '2024-04-02 14:05:00')
    """)

    Iceberg.lastAvailablePartition(tableName, "created_at", threeHourSpec) shouldBe Some("2024-04-02-09-00")
  }
