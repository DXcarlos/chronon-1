...
class FormatTest extends SparkTestBase with Matchers {
  ...
  // Edge case: no clustering key exists, effectiveColumn falls back to caller's "ds" (lowercase),
  // but the storage format returns column names uppercased ("DS"). The hook must still match.
  it should "return results when resolvePartitionColumn falls back to caller column but storage key differs only in case" in {
    val fmt = new Format {
      override def supportSubPartitionsFilter = false
      override def partitions(tableName: String, partitionFilters: String)(implicit ss: SparkSession) =
        List(Map("DS" -> "2023-01-01"), Map("DS" -> "2023-01-02"))
      // no-op: simulates no clustering key found, effectiveColumn stays as caller's "ds"
      override protected def resolvePartitionColumn(tableName: String, partitionColumn: String)(implicit ss: SparkSession) =
        partitionColumn
    }
    // Would return empty with an exact .get("ds") lookup — requires case-insensitive matching
    fmt.primaryPartitions("db.table", "ds", "")(spark) shouldBe empty
  }
