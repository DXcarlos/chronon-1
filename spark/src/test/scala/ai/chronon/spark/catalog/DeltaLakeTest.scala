...
class DeltaLakeTest extends AnyFlatSpec with BeforeAndAfterAll {
  ...
  it should "floor off-grid timestamps to the sub-daily grid in Delta log stats boundaries" in {
    ...
    try {
      ...
      // raw max for the sensor's settle gate: millisecond precision, not grid-floored
      DeltaLake.maxTimestampMillis(tableName, "created_at", threeHourSpec) shouldBe Some(1704117900000L)
  ...
  it should "apply readiness semantics to epoch-millis numeric columns in Delta log stats" in {
    ...
    try {
      ...
      DeltaLake.maxTimestampMillis(tableName, "event_ms", threeHourSpec) shouldBe Some(1704117900000L)
  ...
  it should "fall back to scanning when Delta log stats do not cover the timestamp column" in {
    ...
    try {
      ...
      // stats can't answer, so max ts falls back to the scan tier: 2024-02-03 12:00:00 UTC
      DeltaLake.maxTimestampMillis(tableName, "created_at", PartitionSpec.daily) shouldBe Some(1706961600000L)
    } finally {
      spark.sql(s"DROP TABLE IF EXISTS $tableName")
      spark.sql(s"DROP DATABASE IF EXISTS $dbName")
    }
