...
class BatchNodeRunner(node: Node, tableUtils: TableUtils, api: Api) extends NodeRunner {
  ...
  private def requiredInputTables(metadata: MetaData): Seq[String] = {
    if (!metadata.isSetExecutionInfo || !metadata.executionInfo.isSetTableDependencies) {
      return Seq.empty
    }

    metadata.executionInfo.getTableDependencies.asScala
      .filterNot(td => td.isSetIsSoftNodeDependency && td.isSoftNodeDependency)
      .flatMap(td => Option(td.tableInfo).flatMap(tableInfo => Option(tableInfo.table)))
      .distinct
      .toSeq
  }

  private def checkInputTablesExist(metadata: MetaData): Unit = {
    val missingInputTables = requiredInputTables(metadata).filterNot(tableUtils.tableReachable(_, ignoreFailure = true))
  ...
  private def logUnavailableInputPartitions(metadata: MetaData, range: PartitionRange): Unit = {
    Try(computeInputTablePartitionStatuses(metadata, range, tableUtils).filterNot(_.ready).toSeq) match {
      case Success(unavailableTables) if unavailableTables.nonEmpty =>
        val unavailableTablesMessage = unavailableTables
          .map { tps =>
  ...
  def runFromArgs(
    ...
    Try {
      ...
      logUnavailableInputPartitions(metadata, range)
