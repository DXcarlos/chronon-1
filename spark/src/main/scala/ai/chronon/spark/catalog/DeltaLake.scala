...
import org.apache.spark.sql.functions.{coalesce, col, count, from_json, lit, max, min, timestamp_millis, to_timestamp, when}
import org.apache.spark.sql.types.{DataType, DateType, LongType, MapType, NumericType, StringType, StructField, StructType, TimestampType}
...
case object DeltaLake extends Format {
  ...
  private[catalog] def statsDateRange(tableName: String, columnName: String, partitionSpec: PartitionSpec)(implicit
    ...
    Try {
      ...
      // raw epoch millis, grid-floored via spec.at in Scala: date_format would emit off-grid
      // labels for sub-daily or offset-anchored specs (e.g. '...-05-30' from MAX(ts)=05:30)
