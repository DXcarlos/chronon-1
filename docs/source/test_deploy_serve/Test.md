...
For example, if you have a `Join` named `purchase_training_set` defined at `joins/team/join.py`,
then the compiled file will be at `compiled/joins/team/join.purchase_training_set`.
...
```sh
zipline run compiled/joins/team/join.purchase_training_set --mode=backfill --start-ds=2022-07-02 --end-ds=2022-07-10
```
...
```sh
zipline run compiled/group_bys/team/group_by.purchase_features --mode=backfill --start-partition=2022-07-02 --end-ds=2022-07-10
```
...
```bash
zipline run compiled/group_bys/your_group_by.purchase_features --mode upload --ds 2023-12-01
```

and then to actually upload to your KV store: 
```bash
zipline run compiled/group_bys/your_group_by.purchase_features --mode upload-to-kv --ds 2023-12-01
```
...
```sh
zipline run compiled/group_bys/your_group_by.purchase_features --mode streaming deploy --version-check --disable-cloud-logging [--latest-savepoint| --no-savepoint | --custom-savepoint]
```
...
```bash
zipline run compiled/joins/your_join.purchase_training_set --mode metadata-upload --ds 2023-12-01
```
...
```bash
zipline run compiled/group_bys/your_group_by.purchase_features --mode metadata-upload --ds 2023-12-01
```
...
```bash
zipline run compiled/joins/your_join.purchase_training_set --mode=fetch -k '{"user_or_visitor":"u_106386039"}' --name <JOIN_NAME>
```
