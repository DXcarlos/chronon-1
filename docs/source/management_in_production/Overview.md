...
* Data quality: Chronon produces data in batch, stored in Iceberg tables, and online data produced in realtime stored in the KV store. Data quality metrics are computed and exposed on both of these. For Join feature sets, setting `enable_stats_compute=True` on the Join definition triggers a downstream data quality job that computes feature statistics and surfaces them in Zipline under **Offline > Metrics**. See [Data Quality documentation](./data_quality.md).
