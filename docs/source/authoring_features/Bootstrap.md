...
`payments_driver.logged_driver` is a StagingQuery based on the log table of the production Join, optionally unioned with historical data before logging is available. `logged_driver_source` is a HiveEventSource based on this StagingQuery, used as `left` of the production Join.
...
logged_driver_source = EventSource(
	table=payments_driver.logged_driver.table,
	query=Query(
		selects=select(
			rng='rng',
...
`production_model` presents a production model feature set.
`left` is a wrapped table based on `production_model`'s own logging table and historical data.
`PRODUCTION_FEATURES`/`PRODUCTION_EXT_FEATURES` are file-level definitions to be shared across joins.
`bootstrap_from_log=True` enables bootstrap from `production_model`'s own logging table.

```python
PRODUCTION_FEATURES = [...]
PRODUCTION_EXT_FEATURES = [
    ExternalPart(
        ContextualSource(
            fields=[
...
production_model = Join(
    online=True,
    left=logged_driver_source,
    right_parts=PRODUCTION_FEATURES,
    online_external_parts=PRODUCTION_EXT_FEATURES,
    bootstrap_from_log=True,
    row_ids=ROW_IDS
)
```

To enable the next round of feature experimentation, we decide to create a new source based on the output table of `production_model`.
In particular, we would like the label information from `production_model`'s output table in order to run downsampling on the training examples.

Note: `rng` is an example for a column that clients can include in the StagingQuery `payments_driver.logged_driver` in order to facilitate downsampling for sampling purpose and persisted for multiple runs

```python
experiment_source = EventSource(
    table=production_model.table,
    query=Query(
...
`experiment_model` is a join created purely for offline experimental features, and its output table will contain production features from `production_model` plus new experimental features. For production features, we reuse data from `production_model` without recomputation via bootstrap.

```python
experiment_model = Join(
	online=False,
	left=experiment_source,
	right_parts=PRODUCTION_FEATURES+EXPERIMENTAL_FEATURES,
	bootstrap_parts=[
		BootstrapPart(
			table=production_model.table
		)
...
After experimentation we are ready to productionize the candidate feature set and make it online.
Similar to `logged_driver_source`, we create an `expanded_driver_source` based on a new StagingQuery `payments_driver.expanded_driver`.

```python
expanded_driver_source = EventSource(
	table=payments_driver.expanded_driver.table,
	query=Query(
		selects=select(
			rng='rng',
			**{pk: pk for pk in ENTITY_KEYS}
...
`expanded_model` is the join to serve the updated feature set (containing production and experimental features).
Note it has 3 bootstrapping component:
- `bootstrap_from_log` will populating ongoing data from logging table
- `production_model.table` will carry over production feature data
- `experiment_model.table` will carry over partially backfilled experimental features
