...
```python
purchases_with_users = StagingQuery(
    query="""
        SELECT
            a.id_user,
            a.ts,
...
purchase_training_set = Join(
    left=EventSource(
        table=file_name.purchases_with_users.table
        ...
    )
)
```
