...
* For sub-daily StagingQueries, every `TableDependency` must still declare its own `partition_interval`/`partition_offset`. `time_partitioned=True` changes readiness sensing only; Chronon does not infer an upstream cadence from the downstream output grid.
