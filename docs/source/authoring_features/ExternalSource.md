...
```python
model_with_custom_bootstrap = Join(
    left=...,
    right_parts=[...],
    online_external_parts=[
...
payment_fraud_features = Join(
    ...
    right_parts=[
        JoinPart(group_by=payment_history.purchase_features),
        JoinPart(group_by=user_profile.user_features)
    ],
    online_external_parts=[
