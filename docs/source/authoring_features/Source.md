...
* For sub-daily outputs, `time_partitioned=True` does not declare or inherit cadence. Declare the source `partition_interval`/`partition_offset` explicitly so Python can validate the upstream grid.
